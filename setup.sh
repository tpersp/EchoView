#!/usr/bin/env bash
#
# setup.sh - "It Just Works" for the new PySide6 + FastAPI EchoView
#
...
#   7) Creates systemd services for echoview.py + controller
...
if [[ "$AUTO_UPDATE" == "false" ]]; then
  ...
  echo "  7) Create systemd services for echoview.py and the controller"
...
# -------------------------------------------------------
# 7) Create systemd services for echoview + controller
# -------------------------------------------------------
echo
...
ExecStart=/usr/bin/python3 $VIEWER_HOME/echoview.py
...
[Unit]
Description=EchoView FastAPI Web Controller
...
if [[ "$AUTO_UPDATE" == "false" ]]; then
  ...
  echo "    - echoview.service running echoview.py                     "
